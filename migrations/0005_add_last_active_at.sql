...
-- Set initial last_active_at to NOW() for existing users
UPDATE users SET last_active_at = NOW() WHERE last_active_at IS NULL;

-- Set initial last_active_at to NOW() for existing talent profiles
UPDATE talent_profiles SET last_active_at = NOW() WHERE last_active_at IS NULL;
